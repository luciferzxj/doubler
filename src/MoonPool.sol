...
contract MoonPool is IMoonPool, ERC20, AccessControlEnumerable, ReentrancyGuard {
    ...
    //     require(msg.sender==_poolCfg.doubler);
    // }
    ...
    function input(uint256 _doublerId, SignatureParams memory datas) external {
        ...
        uint256 spendAmount = ISwapAggregator(_poolCfg.aggregator).outputSwap(_poolCfg.asset, doubler.asset,datas.amountOut,datas.maxAmountIn);
        {
            uint256 spenAmount = spendAmount*10**18/10**IERC20Metadata(_poolCfg.asset).decimals();
            uint256 feedPrice = IFastPriceFeed(_poolCfg.pricefeed).getPrice(doubler.asset);
            uint256 priceAmount = datas.amountOut*feedPrice;
            uint256 ratio = priceAmount>spenAmount? priceAmount/(priceAmount-spenAmount):priceAmount/(spenAmount-priceAmount);
            require(ratio>=10,"price error");
        }
        IERC20(doubler.asset).approve(_poolCfg.doubler,datas.amountOut);
        _input(spendAmount, datas.amountOut, _doublerId, doubler, doubler.asset);
    ...
    //     returnAmount = ISwapRouter(_poolCfg.uniswapV3Router).exactInputSingle(
    //         desc
    //     );
    ...
    function gain(uint256 _tokenId) external {
        ...
        uint256 returnAmount = ISwapAggregator(_poolCfg.aggregator).inputSwap(pool.asset,_poolCfg.asset,amount, 10);
        {
            uint256 retAmount = returnAmount*10**18/10**IERC20Metadata(_poolCfg.asset).decimals();
            uint256 feedPrice = IFastPriceFeed(_poolCfg.pricefeed).getPrice(pool.asset);
            uint256 priceAmount = amount*feedPrice;
            uint256 ratio = priceAmount>retAmount? priceAmount/(priceAmount-retAmount):priceAmount/(retAmount-priceAmount);
            require(ratio>=10,"price error");
        }
