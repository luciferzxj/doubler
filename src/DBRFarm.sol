    ...
    OwnableUpgradeable
{
    ...
    function endDoubler(uint256 _doublerId) external nonReentrant {
        ...
        uint256 reward = _endReward;
        if (reward > 0) {
            require(IERC20(_dbrAsset).balanceOf(_farmWallet) >= reward, 'farmWallet balance error');
            IERC20(_dbrAsset).safeTransferFrom(_farmWallet, _msgSender(),  reward);
        }
        emit EndDoubler(_doublerId,_msgSender(), reward);
    }
