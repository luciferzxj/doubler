...
contract UniV2Router {
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ...
                : _to;
            UniswapV2Pair(getPair[input][output]).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
    ...
    function getReserves(
        ...
        (uint reserve0, uint reserve1) = UniswapV2Pair(getPair[tokenA][tokenB])
            .getReserves();
...
contract swapRouter {
    ...
    function exactInput(
        ...
        if (params.path.length == 96) {
            path = new address[](2);
            (address token1, , address token2) = abi.decode(
                params.path,
            ...
            path = new address[](3);
            (address token1, , address token2, , address token3) = abi.decode(
                params.path,
        ...
        require(
            amounts[amounts.length - 1] >= params.amountOutMinimum,
            "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        IERC20(path[0]).transferFrom(
            msg.sender,
    ...
    function exactOutput(
        ...
        if (params.path.length == 96) {
            path = new address[](2);
            (address token2, , address token1) = abi.decode(
                params.path,
                (address, uint24, address)
            );
            path[0] = token1;
            ...
            path = new address[](3);
            (address token3, , address token2, , address token1) = abi.decode(
                params.path,
                (address, uint24, address, uint24, address)
            );
            path[0] = token1;
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? getPair[output][path[i + 2]]
                : _to;
            UniswapV3Pair(getPair[input][output]).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
